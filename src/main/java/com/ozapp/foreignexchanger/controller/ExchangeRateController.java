...
public class ExchangeRateController {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Exchange rate successfully listed.",
                                        content = {@Content(mediaType = "application/json", schema = @Schema(implementation = Double.class))}),
            @ApiResponse(responseCode = "400", description = "Invalid source or target currency is supported.", content = @Content)})
    @GetMapping(value = "/{sourceCurrency}/{targetCurrency}")
    public ResponseEntity<?> getExchangeRate(@PathVariable("sourceCurrency") String sourceCurrency,
